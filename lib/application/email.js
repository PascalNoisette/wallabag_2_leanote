    ...
    return new Promise((resolve, reject) => {
        ...
        if (message.mail.attachments && message.mail.attachments.length>0) {
            ...
            message.mail.attachments.forEach(attachment => {
                var i = message.files.length;
                message['files' + i + 'FileId']= attachment.filename;
                message['files' + i + 'LocalFileId']= "" + i;
                message['files' + i + 'Type']=attachment.contentType;
                message['files' + i + 'Title']=attachment.filename;
                message['files' + i + 'HasBody']=true;
                message['files' + i + 'IsAttach']=true;
                message['fileDatas' + i] = attachment.content;
            });
        }
        resolve(message);
    });
